...
pynightmares strange_lib      # Демонстрирует самую странную либу
pynightmares strange_gui      # Открывает Tk-окно с той же лябой
```
...
- **Странная либа**: Непредсказуемые функции, которые притворяются полезными.
- **Странный GUI**: Небольшое Tk-приложение, которое запускает все команды из скриптов и делает вид, что так и надо.
